...
def search_stablecoin_pairs(
    ...
    for query_idx, query in enumerate(search_queries, 1):
        try:
            ...
            pairs = data.get("pairs", [])
            logger.debug(f"[搜索 {stable_symbol}] 查询 '{query}' 返回 {len(pairs)} 个交易对")
    ...
    stats = {
        ...
        "filtered_chain": 0,
        "filtered_not_stable": 0,
        "filtered_not_target": 0,
        "filtered_liquidity": 0,
        "filtered_no_address": 0,
    ...
    for candidate in all_candidates:
        chain_id_normalized = candidate["chain_normalized"]
        base_symbol = candidate["base_token"]["symbol"]
        quote_symbol = candidate["quote_token"]["symbol"]
        ...
        if base_symbol not in STABLE_SYMBOLS or quote_symbol not in STABLE_SYMBOLS:
            stats["filtered_not_stable"] += 1
            logger.debug(
                f"[搜索 {stable_symbol}] 跳过非稳定币交易对: {base_symbol}/{quote_symbol} "
                f"on {chain_id_normalized}"
            )
            continue
        
        # 3. 目标稳定币检查
        if stable_symbol.upper() not in [base_symbol, quote_symbol]:
            stats["filtered_not_target"] += 1
            logger.debug(
                f"[搜索 {stable_symbol}] 跳过不包含目标稳定币的交易对: {base_symbol}/{quote_symbol} "
                f"on {chain_id_normalized}"
            )
            continue
        ...
        if liquidity_usd < min_liquidity_usd:
            ...
            logger.debug(
                f"[搜索 {stable_symbol}] 跳过流动性不足的交易对: {base_symbol}/{quote_symbol} "
                f"on {chain_id_normalized} | 流动性: ${liquidity_usd:,.0f} < 要求: ${min_liquidity_usd:,.0f}"
            )
            continue
        
        # 5. 地址检查
        if not pair_address:
            stats["filtered_no_address"] += 1
            logger.debug(
                f"[搜索 {stable_symbol}] 跳过无 pair_address 的交易对: {base_symbol}/{quote_symbol} "
                f"on {chain_id_normalized}"
            )
            continue
        
        # 6. 去重检查
        pair_key = (chain_id_normalized, str(pair_address).strip())
        if pair_key in seen_pairs:
            stats["filtered_duplicate"] += 1
            logger.debug(
                f"[搜索 {stable_symbol}] 跳过重复交易对: {base_symbol}/{quote_symbol} "
                f"on {chain_id_normalized} | pair_address: {pair_address[:30]}..."
            )
            continue
        ...
        if legitimacy["risk_level"] == "danger":
            stats["filtered_danger"] += 1
            logger.warning(f"检测到假币，已过滤: {base_symbol}/{quote_symbol} on {chain_id_normalized}")
        ...
        stats["passed"] += 1
        logger.debug(
            f"[搜索 {stable_symbol}] ✅ 添加交易对: {base_symbol}/{quote_symbol} "
            f"on {chain_id_normalized} | 流动性: ${liquidity_usd:,.0f} | "
            f"价格: ${candidate['price_usd']:.6f if candidate['price_usd'] else 'N/A'} | "
            f"pair_address: {pair_address[:30]}..."
        )
    ...
    logger.info(
        ...
        f"流动性不足({stats['filtered_liquidity']}) | "
        f"无地址({stats['filtered_no_address']}) | "
        f"重复({stats['filtered_duplicate']}) | "
        f"假币({stats['filtered_danger']}) | "
    ...
    for idx, symbol in enumerate(stable_symbols, 1):
        try:
            ...
            if pairs:
                by_chain = {}
                for p in pairs:
                    chain = p.get("chain", "未知")
                    by_chain[chain] = by_chain.get(chain, 0) + 1
                chains_info = ", ".join([f"{k}({v})" for k, v in sorted(by_chain.items())])
                logger.info(f"[自动采集] {symbol} 找到 {len(pairs)} 个交易对 [链分布: {chains_info}]")
            else:
                logger.info(f"[自动采集] {symbol} 找到 0 个交易对（可能：无交易对/流动性不足/不在选择的链上）")
        except Exception as e:
            error_count += 1
    ...
    while True:
        ...
        try:
            ...
            if should_send_heartbeat():
                ...
                # 分离脱锚和正常的稳定币
                alert_list = []  # 脱锚的稳定币（详细显示）
                normal_summary = []  # 正常的稳定币（汇总显示）
                
                for symbol in sorted(by_symbol.keys()):
                    symbol_statuses = by_symbol[symbol]
                    alert_count = sum(1 for s in symbol_statuses if s.get("is_alert", False))
                    total_chains = len(symbol_statuses)
                    
                    if alert_count > 0:
                        # 脱锚的稳定币：详细显示
                        chains_info = []
                        for s in sorted(symbol_statuses, key=lambda x: x.get("chain", "")):
                        ...
                        if chains_info:
                            alert_list.append(f"  ⚠️ {symbol}: {', '.join(chains_info)}")
                    else:
                        # 正常的稳定币：只显示汇总
                        # 计算价格范围和链数
                        prices = [s.get("price", 0) for s in symbol_statuses if s.get("price", 0) > 0]
                        if prices:
                            ...
                            price_range = "N/A"
                        normal_summary.append(f"{symbol}({total_chains}链,{price_range})")
                ...
                hb_msg = (
                    ...
                    f"⏰ 时间: {hb_time}\n"
                    f"📊 监控统计:\n"
                    f"  • 监控池: {len(statuses)}个 | 链: {chain_count}条 | 稳定币: {symbol_count}种\n"
                    f"  • 脱锚: {sum(1 for s in statuses if s['is_alert'])}个 | "
                    f"累计告警: {total_alerts}次 | 套利通知: {total_arb_opps}次\n"
                    f"  • Server酱: {serverchan_count}/{MAX_DAILY_SENDS}条 (剩余{serverchan_remaining}条)\n"
                )
                
                # 添加脱锚清单（优先显示）
                if alert_list:
                    hb_msg += f"\n⚠️ 脱锚清单 ({len(alert_list)}个):\n"
                    # 限制脱锚清单最多显示15个，避免过长
                    if len(alert_list) > 15:
                        hb_msg += "\n".join(alert_list[:15])
                        hb_msg += f"\n  ... 还有 {len(alert_list) - 15} 个脱锚项"
                    else:
                        hb_msg += "\n".join(alert_list)
                
                # 添加正常稳定币汇总（紧凑显示）
                if normal_summary:
                    hb_msg += f"\n✅ 正常稳定币 ({len(normal_summary)}种):\n"
                    # 将正常稳定币分组显示，每行显示多个
                    chunk_size = 5  # 每行显示5个
                    for i in range(0, len(normal_summary), chunk_size):
                        chunk = normal_summary[i:i+chunk_size]
                        hb_msg += f"  {', '.join(chunk)}\n"
                
                # 如果都没有
                if not alert_list and not normal_summary:
                    hb_msg += "\n  （暂无监控项）"
